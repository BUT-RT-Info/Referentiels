...
exemple être liées à l'infrastructure de son réseau (état des
équipements, des machines) ou aux utilisateurs. Généralement obtenues
sous forme brute, elles sont ensuite traitées avec des objectifs très
variés (nettoyage des données, extraction d'informations comptables,
archivage, \ldots) pour être réutilisées à d'autres fins ou être
présentées dans des vues synthétiques. Ces traitements peuvent être
récurrents (mensualisation de bilan, sauvegarde de données périodique,
\ldots) et gagnent à être automatisés. Le professionnel R\&T doit donc
développer des scripts ou des programmes pour gérer de façon efficace le
traitement de ces données.\\[3pt]}
